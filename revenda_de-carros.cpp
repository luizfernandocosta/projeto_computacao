...
//Zerando o CPF do comprador para evitar lixo
...
if(entrouCarro == 's' || entrouCarro == 'S'){
  ...
  if(carrosCadastrados != 0 && historicoDeOperacoes != 0){
    ...
    fprintf(historicoDeOperacoes, "Nova venda cadastrada\nNome do comprador: %s\nCPF: %s\nMarca do carro comprado: %s\nModelo do carro comprado: %s\nCor do veiculo comprado: %s\nAno: %d\nValor: %.2f\nComprador entrou com carro para troca?: Sim\nMarca do carro para troca: %s\nModelo do carro para troca: %s\nCor: %s\nAno: %d\nValor: %.2f\nNome do vendedor: %s\nGratificacao: %.2f", comprador.nome, compradorCPF, carro.marca, carro.modelo, carro.cor, carro.ano, carro.valorDoCarro, carroParaTroca.marca, carroParaTroca.modelo, carroParaTroca.cor, carroParaTroca.ano, carroParaTroca.valorDoCarro, vendedor.nome, gratificacao);
  ...
  if(carrosCadastrados != 0 && historicoDeOperacoes != 0){
    fprintf(historicoDeOperacoes, "Nova venda cadastrada\nNome do comprador: %s\nCPF: %s\nMarca do carro comprado: %s\nModelo do carro comprado: %s\nCor do veiculo comprado: %s\nAno: %d\nValor: %.2f\nComprador entrou com algum carro para troca?: Nao\nNome do vendedor: %s\nGratificacao: %.2f", comprador.nome, compradorCPF, carro.marca, carro.modelo, carro.cor, carro.ano, carro.valorDoCarro, vendedor.nome, gratificacao);
    fprintf(historicoDeOperacoes, "\n\n");
    printf("\nVenda cadastrada com sucesso!\n");
    fclose(historicoDeOperacoes);
  } else {
...
if(carrosCadastrados != 0 && historicoDeOperacoes != 0){
  ...
  fprintf(historicoDeOperacoes, "Novo carro cadastrado\nMarca: %s\nModelo: %s\nCor: %s\nAno: %d\nValor: %.2f", carro.marca, carro.modelo, carro.cor, carro.ano, carro.valorDoCarro);
  fprintf(historicoDeOperacoes, "\n\n");
  fclose(historicoDeOperacoes);
} else {
  printf("Nao foi possivel abrir arquivo!\n");
